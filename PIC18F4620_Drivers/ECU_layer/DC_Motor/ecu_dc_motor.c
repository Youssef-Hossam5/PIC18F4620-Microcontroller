 ...
 * Author: lenovo
 *
 * Created on March 4, 2024, 4:42 PM
 */
...
Std_ReturnType dc_motor_initialize(const dc_motor_t *_dc_motor){
    ...
    if(NULL == _dc_motor){
        ...
        gpio_pin_direction_initialize(&(_dc_motor->dc_motor_pin[DC_MOTOR_PIN1]));
        gpio_pin_direction_initialize(&(_dc_motor->dc_motor_pin[DC_MOTOR_PIN2]));
    }
    return ret;
}
