 ...
 * Author: lenovo
 *
 * Created on July 18, 2024, 2:17 PM
 */
...
Std_ReturnType Data_EEPROM_WriteByte(uint16 bAdd, uint8 bData){
    ...
    /*  Disable interrupts based on INTERRUPT PRIORITY LEVELS  */
...
Std_ReturnType Data_EEPROM_ReadByte(uint16 bAdd, uint8 *bData){
    ...
    if(NULL == bData){
        ...
        NOP(); /* NOPs may be required for latency at high frequencies (>20 MHz crystal) */
